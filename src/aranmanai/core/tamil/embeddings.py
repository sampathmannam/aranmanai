...
class TextEmbedder:
    ...
    def embed_batch(self, texts: list[str], batch_size: int = 32) -> list[Embedding]:
        ...
        return [Embedding(text=t, vector=v, model=self.model_name) for t, v in zip(texts, vecs)]
...
def embed_text(text: str, model_name: Optional[str] = None) -> Embedding:
    """Convenience: embed a single text using default model."""
    return TextEmbedder(model_name=model_name).embed(text)


def embed_batch(texts: list[str], model_name: Optional[str] = None, batch_size: int = 32) -> list[Embedding]:
