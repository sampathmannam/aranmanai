...
def test_audit_hash_chain_walks_cleanly(temp_dir):
    ...
    with session_scope() as db:
        ...
        for i in range(5):
            record_audit(
                db, actor_id=u.id, action=f"test.action.{i}",
...
def test_audit_chain_detects_tampering(temp_dir):
    ...
    with session_scope() as db:
        ...
        e = db.query(AuditLog).order_by(AuditLog.id.asc()).offset(1).first()
        e.action = "tampered.action"
    is_valid, n = verify_audit_chain(db)
    # Chain breaks at entry 2; verify_audit_chain returns n=1 (only the first valid entry)
    assert is_valid is False
    assert n == 1
    # And the total number of entries in the DB is still 3
    with session_scope() as db:
        from src.aranmanai.models import AuditLog
...
def test_audit_genesis_hash_is_zeros(temp_dir):
    """First audit entry's prev_hash should be 64 zeros (genesis)."""
    init_db()
    with session_scope() as db:
        u = User(name="genesis_test", role="IO", district="Vellore", password_hash="x")
        db.add(u); db.flush()
        record_audit(
            db, actor_id=u.id, action="genesis.test",
